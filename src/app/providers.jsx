  ...
  if (!mounted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0c0604] text-white">
         ...
         <p className="text-orange-200/60 tracking-widest text-sm uppercase">Loading RecipeHub...</p>
      </div>
    );
  }
    ...
    <ThemeProvider attribute="class" defaultTheme="dark" enableSystem>
      {isPending && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#0c0604]/80 backdrop-blur-sm text-white">
           ...
           <p className="text-orange-200/80 tracking-widest text-sm font-semibold uppercase animate-pulse">Authenticating...</p>
        </div>
      )}
      {children}
    </ThemeProvider>
